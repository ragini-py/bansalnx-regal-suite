...
export const Route = createFileRoute("/checkout")({
  ...
  head: () => ({
    meta: [
      { title: "Checkout | Bansal-nx" },
      { name: "description", content: "Complete your Bansal-nx order securely." },
      { property: "og:title", content: "Checkout | Bansal-nx" },
      { property: "og:description", content: "Complete your Bansal-nx order securely." },
    ],
  }),
});
  ...
  async function handlePlaceOrder() {
    ...
    if (useNewAddress) {
      ...
      address = saveNewAddress
        ? addAddress({ ...parsed, isDefault: (user?.addresses.length ?? 0) === 0 })
        : { id: "guest-address", ...parsed, isDefault: false };
    } else {
    ...
    // Simulated payment state machine: a real integration hands off to the
    // Razorpay Checkout widget here and confirms success only via a signed
    // server-side webhook — never from the client.
    await new Promise((resolve) => setTimeout(resolve, 1600));

    const order = placeOrder({ address, paymentMethod, email, phone });
