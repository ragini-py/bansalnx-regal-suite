...
import { resetPasswordRequest } from "@/lib/api/password-reset";
...
export function ResetPasswordPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      await resetPasswordRequest({ token: token!, ...result.data });
      setDone(true);
    } catch (err) {
      if (isAxiosError(err) && err.response?.status === 401) {
        setExpired(true);
      } else {
        setFormError(extractApiErrorMessage(err, "Something went wrong. Please try again."));
      }
    } finally {
      setLoading(false);
    }
                  ...
                  {values.password && (
                    <div className="mt-2">
                      ...
                      <p id="password-hint" className="mt-1 text-[11px] text-muted-foreground">
                        Strength: {strengthLabel} — use 8+ characters with a mix of letters, numbers
                        and symbols.
                      </p>
                    </div>
                  )}
