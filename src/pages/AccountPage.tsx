...
export function AccountPage() {
  ...
  return (
    <AccountLayout
      ...
      description={activeTab === "overview" ? `Welcome back, ${user.firstName}.` : undefined}
    >
      <div>
        {/* 1. OVERVIEW */}
        {activeTab === "overview" && (
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              >
                <div>
                  ...
                  <p className="mt-2 font-display font-bold text-2xl text-slate-900">
                    {myOrders.length}
                  </p>
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                {myOrders.length > 0 && (
                  <Link
                    to="/account/orders"
                    className="text-xs uppercase tracking-[0.15em] text-gold-deep link-underline"
                  >
                    View All Orders
                  </Link>
                )}
              ...
              {myOrders.length === 0 ? (
                ...
                <ul className="mt-4 space-y-3">
                  {myOrders.slice(0, 3).map((o) => (
                    <li
                      key={o.id}
                      className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border border-border/70 bg-card p-5"
                    >
                      <div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {user.addresses.map((addr) => (
                <div
                  key={addr.id}
                  className="border border-border/80 bg-card p-5 flex flex-col justify-between"
                >
                  <div>
                    <div className="flex items-center justify-between">
                      <span className="text-xs uppercase tracking-[0.15em] font-medium text-gold-deep">
                        {addr.label}
                      </span>
                      {addr.isDefault && (
                        <Badge
                          variant="outline"
                          className="border-gold/50 text-gold-deep rounded-none text-[10px]"
                        >
                          Default
                        </Badge>
                      )}
              ...
              <Dialog open={addAddressOpen} onOpenChange={setAddAddressOpen}>
                <DialogContent className="max-w-lg rounded-none border border-border bg-background p-6">
                  ...
                  >
                    ...
                    <Button type="submit" variant="luxe" className="sm:col-span-2 mt-2">
                      Save Address
                    </Button>
                  </form>
                </DialogContent>
        ...
        {activeTab === "coupons" && (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Copy promo codes to claim discounts at checkout.
              </p>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {activeCoupons.map((c) => (
                <div
                  key={c.id}
                  className="border border-gold/40 bg-gold/5 p-5 flex flex-col justify-between"
                >
                  <div>
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-base font-medium tracking-wider text-gold-deep">
                        {c.code}
                      </span>
                    ...
                    <p className="mt-2 text-xs text-muted-foreground">
                      Valid on orders above {formatINR(c.minOrder ?? 0)} · Expires{" "}
        ...
        {activeTab === "profile" && (
          <div className="max-w-xl space-y-6">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Your account details and contact preferences.
              </p>
            </div>
            ...
            <div className="border border-border/80 bg-card p-6 space-y-4">
              <div>
                ...
                <Input
                  ...
                  disabled
                  className="mt-1 rounded-none bg-muted/40"
                />
              </div>
              <div>
